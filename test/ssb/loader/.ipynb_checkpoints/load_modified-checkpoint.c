...
void supplier (FILE *fp, char *outName){
  ...
  // opens 7 output files and initializes them with the names specified by "outName" and an index number.
  // 7 because one column per file
  for(i=0;i<7;i++){
  ...
  tupleRemain = tupleNum;
    ...
    // BLOCKNUM should be the max number of tuples in the block
  if(tupleNum > BLOCKNUM)      
    tupleUnit = BLOCKNUM;
  else
    tupleUnit = tupleNum;
  // this is the number of tuples in the block
...
/*  header.blockSize = header.tupleNum * sizeof(int);*/
  ...
  while(fgets(buf,sizeof(buf),fp)!= NULL){
    ...
    // iterate through string and attributes
    for(i = 0, prev = 0,count=0; buf[i] !='\n';i++){
      if (buf[i] == delimiter){
        ...
        // count is the idx of the current attribute
        switch(count){
           case 0:
            // write to out[0]
            if(writeHeader == 1){
